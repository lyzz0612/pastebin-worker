  ...
  if (item.value === null) {
    ...
    if (metadata.location === "R2") {
      if (!isR2Available(env)) {
        throw new WorkerError(500, `paste '${short}' is stored in R2 but R2 is not configured`)
      }
...
export async function updatePaste(
  ...
  // Determine if the content should be stored in R2
  const shouldUseR2 =
    originalMetadata.location === "R2" || options.isMPUComplete || options.contentLength > parseSize(env.R2_THRESHOLD)!

  // Check if R2 is needed but not available
  if (shouldUseR2 && !isR2Available(env)) {
    throw new WorkerError(
      413,
      `file too large (max ${env.R2_THRESHOLD} without R2 storage). R2 bucket is not configured.`,
    )
  }

  // if the paste is previous on R2, we keep it on R2 to avoid losing reference to it
  const newLocation = shouldUseR2 ? "R2" : "KV"
...
export async function deletePaste(env: Env, pasteName: string, originalMetadata: PasteMetadata): Promise<void> {
  ...
  if (originalMetadata.location === "R2" && isR2Available(env)) {
    await env.R2.delete(pasteName)
  }
}

export async function cleanExpiredInR2(env: Env, controller: ScheduledController) {
  // Skip cleanup if R2 is not configured
  if (!isR2Available(env)) {
    console.log("R2 is not configured, skipping R2 cleanup")
    return
  }
